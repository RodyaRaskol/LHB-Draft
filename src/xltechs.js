...
function AddColortoElement(myElement, myColor) {
  if (!myElement.classList.contains(myColor)) {
...
function InitTechnique(idTech, navType = "Main") {
  ...
  const docDept = xlTechniquesItemsObj.filter(
    (xltech) => xltech.id == idTech )[0].ParentID;
  ...
  const myTech = xlTechniquesItemsObj.filter(
    (technique) => technique.id == idTech
  );
  //Set Indicator Color
  let myColor;
  switch (true) {
  ...
  let myDiv, myLink, myBtn, myLine, myBGColor;
  myDiv = document.getElementById("storeBanner-main");

  myBGColor = myDiv.style.backgroundColor;
  myBGColor = window.getComputedStyle(myDiv).backgroundColor;
  console.log("Hi");
  ...
  for (var i = 0; i < myTech[0].Details.length; i++) {
    ...
    myLine.appendChild(
      document.createTextNode(myTech[0].Details[i].Discussion)
    );
 ...
 // VideoTitleSmall.innerHTML = iconBurger + " " + myTech[0].VideoName;
  ...
  const menusToFill = [
    {
      navType: "Main",
      fncRoot: "XlFunctionsPH",
      prcRoot: "xlProceduresPH",
      tchRoot: "xlTechniquesPH",
      mthRoot: "method-bar-large",
    }
    
  ];
  //Loop Over Items to Fill
  
    const myFNCContainer = document.getElementById("XlFunctionsPH");
    const myPRCContainer = document.getElementById("xlProceduresPH");
    const myTCHContainer = document.getElementById("xlTechniquesPH");
    const myMTHContainer = document.getElementById("xlMethodsPH");
    ...
    myFNCBtn.style.borderColor = null;
    if (myTech[0].fncs.length > 0) {
      myFNCBtn.setAttribute(
        "style",
        "border-Color:" + myBGColor + " !important"
      );
    }
    ...
    for (var i = 0; i < myTech[0].fncs.length; i++) {
      ...
      myBtn.type = "button";
      myBtn.className =
        "w3-button w3-right w3-padding w3-opacity w3-small " + myColor;
    ...
    myPRCBtn.style.borderColor = null;
    if (myTech[0].pros.length > 0) {
      myPRCBtn.setAttribute(
        "style",
        "border-Color:" + myBGColor + " !important"
      );
    }
    ...
    for (var i = 0; i < myTech[0].pros.length; i++) {
      ...
      let myProcedure = xlProcedures.filter(
        (pro) => pro.id == myTech[0].pros[i]
      );
      myBtn = document.createElement("button");
    ...
    myTCHBtn.style.borderColor = null;
    if (myTech[0].tecs.length > 0) {
      // myTCHBtn.classList.add(myColor);

      myTCHBtn.setAttribute(
        "style",
        "border-Color:" + myBGColor + " !important"
      );
    }
    ...
    for (var i = 0; i < myTech[0].tecs.length; i++) {
      ...
      let myTechnique = xlTechniquesItemsObj.filter(
        (tch) => tch.id == myTech[0].tecs[i]
      );
      myBtn = document.createElement("button");
    ...
    myBtn.type = "button";
    myBtn.className =
      "w3-bar-item w3-button w3-border w3-border-white " + myColor;
...
function SetVideoSource(mySRC, btnID, btnColor) {
  ...
  if (mybtn.classList.contains(btnColor)) {
    mybtn.classList.replace(btnColor, "w3-grey");
  }
